...
/**
 * Implements the helicopter. The helicopter appears if the city contains an airport. It usually
 * flies to the location in the city with the highest traffic density, but sometimes flies to other
 * locations.
 */
public class HelicopterSprite extends Sprite {
  ...
  public void moveImpl() {
    ...
    if (getCity().getAcycle() % SOUND_FREQ == 0) {
      ...
      if (getCity().getTrafficDensity(xpos, ypos) > 170 && getCity().getRandom().nextInt(8) == 0) {
        getCity().sendMessageAt(MicropolisMessage.HEAVY_TRAFFIC_REPORT, xpos, ypos);
