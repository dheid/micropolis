...
/**
 * Implements the cargo ship. The cargo ship is created if the city contains a sea port. It follows
 * the river "channel" that was originally generated. It frequently turns around.
 */
public class ShipSprite extends Sprite {
  ...
  private static final int[] BtClrTab = {
    RIVER, CHANNEL, POWERBASE, POWERBASE + 1, RAILBASE, RAILBASE + 1, BRWH, BRWV
  };
  private int newDir;
  private int count;
  ...
  private static boolean tryOther(int tile, int oldDir, int newDir) {
    ...
    return tile == POWERBASE || tile == POWERBASE + 1 || tile == RAILBASE || tile == RAILBASE + 1;
  }
    ...
    if (count <= 0) {
      ...
      for (pem = tem; pem < tem + 8; pem++) {
        ...
        if (getCity().testBounds(xpos, ypos)) {
          ...
          if (t == CHANNEL || t == BRWH || t == BRWV || tryOther(t, getDir(), z)) {
            newDir = z;
            setFrame(turnTo(getFrame(), newDir));
            setDir(z + 4);
            if (getDir() > 8) {
