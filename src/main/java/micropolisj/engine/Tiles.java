...
class Tiles {
  ...
  private static void readTiles() throws IOException {
    ...
    tilesRc.load(
        new InputStreamReader(Tiles.class.getResourceAsStream("/graphics/tiles.rc"), UTF8));
    ...
    for (TileSpec tile : tiles) {
      ...
      if (bi != null) {
        for (int j = 0; j < bi.getMembers().length; j++) {
          ...
          if (tiles[tid].getOwner() == null && (offx != 0 || offy != 0)) {
   ...
   *
   * @return a tile specification, or null if there is no tile with the given number
   */
