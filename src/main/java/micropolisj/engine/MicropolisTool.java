...
/**
 * Enumerates the various tools that can be applied to the map by the user. Call the tool's apply()
 * method to actually use the tool on the map.
 */
public enum MicropolisTool {
  ...
  /**
   * This is the cost displayed in the GUI when the tool is selected. It does not necessarily
   * reflect the cost charged when a tool is applied, as extra may be charged for clearing land or
   * building over or through water.
   */
  public int getCost() {
    return cost;
  }
