...
/**
 * Represents a mobile entity on the city map, such as a tornado or a train. There can be any number
 * present in a city, and each one gets a chance to act on every tick of the simulation.
 */
public abstract class Sprite {
   ...
   *
   * @return integer between 1 and 8, with 1 == north, 3 == east, 5 == south, 7 == west.
   */
  ...
  /**
   * For subclasses to override. Actually does the movement and animation of this particular sprite.
  ...
  /**
   * Destroys whatever is at the specified location, replacing it with fire, rubble, or water as
   * appropriate.
   */
  void destroyTile(int xpos, int ypos) {
    if (!city.testBounds(xpos, ypos)) return;
