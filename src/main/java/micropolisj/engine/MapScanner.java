...
/**
 * Process individual tiles of the map for each cycle. In each sim cycle each tile will get
 * activated, and this class contains the activation code.
 */
class MapScanner extends TileBehavior {
   ...
   *
   * @return an integer between -3000 and 3000. Same meaning as evalResidential.
   */
  ...
  private boolean setZonePower() {
    ...
    boolean newPower =
        getTile() == NUCLEAR || getTile() == POWERPLANT || getCity().hasPower(getXpos(), getYpos());
  ...
  /**
   * Place a 3x3 zone on to the map, centered on the current location. Note: nothing is done if part
   * of this zone is off the edge of the map or is being flooded or radioactive.
   *
  ...
  private void doNuclearPower() {
    ...
    if (!getCity().isNoDisasters()
        && getRandom().nextInt(MELTDOWN_TAB[getCity().getGameLevel()] + 1) == 0) {
  ...
  /**
   * Regenerate the tiles that make up the zone, repairing from fire, etc. Only tiles that are not
   * rubble, radioactive, flooded, or on fire will be regenerated.
   *
   * @param zoneCenter the tile value for the "center" tile of the zone
   * @param zoneSize integer (3-6) indicating the width/height of the zone.
   */
  ...
  private void doResidential() {
    ...
    tpop =
        getTile() == RESCLR
            ? getCity().doFreePop(getXpos(), getYpos())
            : residentialZonePop(getTile());
   ...
   *
   * @return integer; positive number indicates good place for house to go; zero or a negative
   *     number indicates a bad place.
   */
   ...
   *
   * @return an integer between -3000 and 3000. The higher the number, the more likely the zone is
   *     to GROW; the lower the number, the more likely the zone is to SHRINK.
   */
   ...
   *
   * @return integer from 0 to 3, 0 is the lowest-valued zone, and 3 is the highest-valued zone.
   */
  ...
  /**
   * Record a zone's population change to the rate-of-growth map. An adjustment of +/- 1 corresponds
   * to one little house. An adjustment of +/- 8 corresponds to a full-size zone.
   *
