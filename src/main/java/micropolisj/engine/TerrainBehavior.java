...
class TerrainBehavior extends TileBehavior {
  ...
  private void doFire() {
    ...
    for (int dir = 0; dir < 4; dir++) {
      if (getRandom().nextInt(8) == 0) {
        ...
        if (!getCity().testBounds(xtem, ytem)) continue;
    ...
    int rate = cov > 100 ? 1 : cov > 20 ? 2 : cov != 0 ? 3 : 10;
  ...
  private void doRoad() {
      ...
      if (getRandom().nextInt(512) == 0) {
        if (!isConductive(getTile())) {
          if (getCity().getRoadEffect() < getRandom().nextInt(32)) {
            if (isOverWater(getTile())) getCity().setTile(getXpos(), getYpos(), RIVER);
            else getCity().setTile(getXpos(), getYpos(), (char) (RUBBLE + getRandom().nextInt(4)));
            return;
          }
        }
      }
    ...
    int tden;
    if (getTile() < LTRFBASE) tden = 0;
    else if (getTile() < HTRFBASE) tden = 1;
    else {
      getCity().setRoadTotal(getCity().getRoadTotal() + 1);
      tden = 2;
    }
    ...
    int newLevel = trafficDensity < 64 ? 0 : trafficDensity < 192 ? 1 : 2;
  ...
  /**
   * Called when the current tile is a road bridge over water. Handles the draw bridge. For the draw
   * bridge to appear, there must be a boat on the water, the boat must be within a certain distance
   * of the bridge, it must be where the map generator placed 'channel' tiles (these are tiles that
   * look just like regular river tiles but have a different numeric value), and you must be a
   * little lucky.
   *
  ...
  private void applyBridgeChange(int[] dx, int[] dy, char[] fromTab, char[] toTab) {
    ...
    for (int z = 0; z < 7; z++) {
      ...
      if (getCity().testBounds(x, y)) {
        if (getCity().getTile(x, y) == fromTab[z] || getCity().getTile(x, y) == CHANNEL) {
