...
public class MainWindow extends JFrame implements CityListener, EarthquakeListener {
  ...
  private MainWindow(Micropolis engine) {
    ...
    overlaysMenu.add(
        makeMapStateMenuItem("menu.overlays.GROWTHRATE_OVERLAY", MapState.GROWTHRATE_OVERLAY));
    overlaysMenu.add(
        makeMapStateMenuItem("menu.overlays.LANDVALUE_OVERLAY", MapState.LANDVALUE_OVERLAY));
    overlaysMenu.add(makeMapStateMenuItem("menu.overlays.CRIME_OVERLAY", MapState.CRIME_OVERLAY));
    overlaysMenu.add(
        makeMapStateMenuItem("menu.overlays.POLLUTE_OVERLAY", MapState.POLLUTE_OVERLAY));
    overlaysMenu.add(
        makeMapStateMenuItem("menu.overlays.TRAFFIC_OVERLAY", MapState.TRAFFIC_OVERLAY));
  ...
  private void makeMenu() {
    ...
    for (int i = GameLevel.MIN_LEVEL; i <= GameLevel.MAX_LEVEL; i++) {
      ...
      menuItem.addActionListener(wrapActionListener(evt -> onDifficultyClicked(level)));
    ...
    autoBudgetMenuItem.addActionListener(wrapActionListener(ev -> onAutoBudgetClicked()));
    ...
    autoBulldozeMenuItem.addActionListener(wrapActionListener(ev -> onAutoBulldozeClicked()));
    ...
    disastersMenuItem.addActionListener(wrapActionListener(ev -> onDisastersClicked()));
    ...
    soundsMenuItem.addActionListener(wrapActionListener(ev -> onSoundClicked()));
    ...
    menuItem.addActionListener(wrapActionListener(ev -> doZoom(1)));
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onInvokeDisasterClicked(Disaster.MONSTER)));
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onPriorityClicked(Speed.SUPER_FAST)));
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onPriorityClicked(Speed.FAST)));
    priorityMenu.add(menuItem);
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onPriorityClicked(Speed.NORMAL)));
    priorityMenu.add(menuItem);
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onPriorityClicked(Speed.SLOW)));
    priorityMenu.add(menuItem);
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onPriorityClicked(Speed.PAUSED)));
    priorityMenu.add(menuItem);
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onViewBudgetClicked()));
    windowsMenu.add(menuItem);

    menuItem = new JMenuItem(strings.getString("menu.windows.evaluation"));
    setupKeys(menuItem, "menu.windows.evaluation");
    menuItem.addActionListener(wrapActionListener(ev -> onViewEvaluationClicked()));
    windowsMenu.add(menuItem);

    menuItem = new JMenuItem(strings.getString("menu.windows.graph"));
    setupKeys(menuItem, "menu.windows.graph");
    menuItem.addActionListener(wrapActionListener(ev -> onViewGraphClicked()));
    windowsMenu.add(menuItem);
    ...
    menuItem.addActionListener(wrapActionListener(ev -> onAboutClicked()));
  ...
  private boolean onSaveCityAsClicked() {
    ...
    try {
      ...
      FileNameExtensionFilter filter1 =
          new FileNameExtensionFilter(strings.getString("cty_file"), EXTENSION);
  ...
  private JToggleButton makeToolBtn(MicropolisTool tool) {
    String iconName =
        strings.containsKey("tool." + tool.name() + ".icon")
            ? strings.getString("tool." + tool.name() + ".icon")
            : "/graphics/tools/" + tool.name().toLowerCase(Locale.ENGLISH) + ".png";
    String iconSelectedName =
        strings.containsKey("tool." + tool.name() + ".selected_icon")
            ? strings.getString("tool." + tool.name() + ".selected_icon")
            : iconName;
    String tipText =
        strings.containsKey("tool." + tool.name() + ".tip")
  ...
  private JToolBar makeToolbar() {
    ...
    JToolBar toolBar =
        new JToolBar(strings.getString("main.tools_caption"), SwingConstants.VERTICAL);
  ...
  private void startTimer() {
    ...
    if (engine.getSimSpeed() == Speed.PAUSED) return;
  ...
  private void showErrorMessage(Throwable e) {
    ...
    if (rv == 0) {
      JOptionPane.showMessageDialog(
          this, detailsPane, strings.getString("main.error_unexpected"), JOptionPane.ERROR_MESSAGE);
    }
    if (rv == 2) {
      rv =
          JOptionPane.showConfirmDialog(
  ...
  public void citySound(Sound sound, CityLocation loc) {
    ...
    boolean offScreen =
        !drawingAreaScroll
            .getViewport()
            .getViewRect()
            .contains(drawingArea.getTileBounds(loc.getX(), loc.getY()));
    if (sound == Sound.HONKHONK_LOW && offScreen) return;
  ...
  private void onAboutClicked() {
    ...
    JOptionPane.showMessageDialog(
        this, inputs, strings.getString("main.about_caption"), JOptionPane.PLAIN_MESSAGE, appIcon);
  }
  ...
  private class DrawingAreaMouseAdapter extends MouseAdapter {
    ...
    private void doQueryTool(int xpos, int ypos) {
      if (!engine.testBounds(xpos, ypos)) return;
