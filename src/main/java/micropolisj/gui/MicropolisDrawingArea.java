...
public class MicropolisDrawingArea extends JComponent implements Scrollable, MapListener {
    ...
    addMouseListener(
        new MouseAdapter() {
          ...
          public void mousePressed(MouseEvent e) {
            if (e.getButton() == MouseEvent.BUTTON2) startDrag(e.getX(), e.getY());
          }

          @Override
          public void mouseReleased(MouseEvent e) {
            if (e.getButton() == MouseEvent.BUTTON2) endDrag();
          }
    ...
    addMouseMotionListener(
        new MouseMotionAdapter() {
          ...
          public void mouseDragged(MouseEvent e) {
            if (dragging) continueDrag(e.getX(), e.getY());
          }
  ...
  public void paintComponent(Graphics g) {
    ...
    for (int y = minY; y < maxY; y++) {
      for (int x = maxX - 1; x >= minX; x--) {
        ...
        if (blinkUnpoweredZones && isZoneCenter(cell) && !m.isTilePowered(x, y)) {
          unpoweredZones.add(new Point(x, y));
          if (blink) cell = LIGHTNINGBOLT;
        }
  ...
  public void setToolCursor(ToolCursor newCursor) {
    if (toolCursor == newCursor) return;
    if (toolCursor != null && toolCursor.equals(newCursor)) return;
  ...
  public Rectangle getTileBounds(int xpos, int ypos) {
    return new Rectangle(xpos * tileWidth, ypos * tileHeight, tileWidth, tileHeight);
  }
